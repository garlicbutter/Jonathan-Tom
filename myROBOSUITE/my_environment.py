...
class MyEnv(SingleArmEnv):
    ...
    def motion_planning(self,action_type,target,**kwargs):
        """
        trajectory planning

        Args:
            action_type(str) : type of action so we can know how to move and where to move
                                It can be from the following options.
                                "grasp" : the eef will grab an object 
                                "move"  : the eef will move to the required pos, ori
                                "insert : the eef will insert an object to the target
            target           :  if action_type is "grasp" : the traget to grab
                                if action_type is "move"  : the required pos, ori, for the eef.
                                if action_type is "insert : the target to insert
        Returns:
            action for the robot. (1*7 array for now)
        """
        assert action_type in ("grasp","move","insert"), "Action type only has following options: 'grasp', 'move', 'insert' "

        action = np.array([0, 0, 0, 0, 0, 0, 0])

        ################### TODO ###################
        # 1. get observables for the gripper
        # 2. get observables for the target
        # 3. proper motion planning scheme (minumum jerk maybe??)
        
        if action_type == "move":
            # target might be array or list of [pos], or [[pos],[ori]] , [pos ori]
            # so we need to do some sanity check.
            action[-1] = 1 if self._check_grasp else 0  # keep grab the thing if we are already grabbing something.
            return action

        if action_type == "insert":
            # be aware that the target might have a desired orientation to be inserted (USB for example)
            return action

        if action_type == "grasp":
            # be aware that the target might have a desired orientation to be grabbed (USB for example)
            return action