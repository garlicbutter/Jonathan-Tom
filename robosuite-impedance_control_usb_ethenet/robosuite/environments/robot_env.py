...
class RobotEnv(MujocoEnv):
    ...
    def action_spec(self):
        ...
        if self.action_space_def == "Impedance_K":
            low = np.ones(self.control_spec) * -np.inf
            high = np.ones(self.control_spec) * np.inf

        elif self.action_space_def == "Compliance":
            low = np.ones(self.control_spec) * -np.inf
            high = np.ones(self.control_spec) * np.inf
    ...
    def action_dim(self):
        ...
        """
        #    TODO - action_dim(self) - see how this function realate to.
    ...
    def _reset_internal(self):
        ...
        """
        #   TODO - change this function?!?! (_action_dim)
    ...
    def _pre_action(self, action, policy_step=False):
        ...
        for idx, robot in enumerate(self.robots):
            ...
            if policy_step:
                ...
                # if abs(robot.recent_ee_pose.current[0] - self.sim.data.get_body_xpos("hole")[0]) < 0.0005 \
                #         and abs(robot.recent_ee_pose.current[1] - self.sim.data.get_body_xpos("hole")[1]) < 0.0005 \
                #         and abs(robot.recent_ee_pose.current[2] - self.sim.data.get_body_xpos("hole")[2]) < 0.005:
                #     desired_vec = self.via_points['p1']
                #     self.checked = 1
                # else:
                #     if self.checked == 1:
                #         desired_vec = self.via_points['p1']
                #     else:
                #         desired_vec = self.via_points['p0']
                # robot.desired_pos, robot.desired_ori = desired_vec[:3], desired_vec[3:]

            desired_vec = self.calc_next_desired_point()
            # robot.control_param = self.action_dict
            robot.desired_pos, robot.desired_ori = desired_vec[0], desired_vec[1]
    ...
    #
    #     """
    #     return super._dict_action(action)