...
from robosuite.utils.mjcf_utils import new_joint, array_to_string, new_site


class ManipulationTaskNoGripperFuncConnector(MujocoWorldBase):
    """
    ...
    def __init__(
        ...
        self.hand_objects = []
        self.merge_hand_objects(mujoco_gripper_objects)
        ...
        for obj_name, obj_mjcf in mujoco_objects.items():
            ...
            if obj_name == 'hole':
                for i, joint in enumerate(obj_mjcf.joints):
                    obj.append(new_site(name="{}_site{}".format(obj_name, i), pos=(0,0,1),size=(0.00005,)))
                self.objects.append(obj)
                self.worldbody.append(obj)
                # obj[0].set("pos", array_to_string(np.array([0.0, 0.0, 0.1])))
                # self.worldbody.find('.//body[@name="table_top"]').append(obj)
            else:
                for i, joint in enumerate(obj_mjcf.joints):
    ...
    def merge_hand_objects(self, mujoco_hand_object, is_visual=False):
        """
        Adds object models to the MJCF model.
        ...
        for obj_name, obj_mjcf in mujoco_hand_object.items():
            assert(isinstance(obj_mjcf, MujocoGeneratedObject) or isinstance(obj_mjcf, MujocoXMLObject))
            self.merge_asset(obj_mjcf)
            # Load object
            if is_visual:
            ...
            for i, joint in enumerate(obj_mjcf.joints):
                obj.append(new_site(name="{}_site{}".format(obj_name, i), pos=(0, 0, 0),size=(0.00005,)))
            self.hand_objects.append(obj)
            for i in range(len(obj)):
                obj[i].set("pos", array_to_string(np.array([-0.025, -0.0, 0.21])))
            # self.worldbody.find('.//body[@name="gripper0_robotiq_85_adapter_link"]').append(obj)
            self.worldbody.find(".//body[@name='robot0_right_hand']").append(obj)
