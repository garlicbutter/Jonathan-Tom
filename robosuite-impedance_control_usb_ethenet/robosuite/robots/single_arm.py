...
class SingleArm(Robot):
    ...
    def reset(self, deterministic=False):
        ...
        """
        #   TODO - reset(self, deterministic=False) - see if self.recent_actions affect the space dim
    ...
    def control(self, action, policy_step=False):
        ...
        # if not self.learn:
        #     self.controller.set_control_param(action , 0)
        # else:
        #     self.controller.set_control_param(action, 1)
        if not self.learn:
            self.controller.set_control_param(action)
            self.learn = True
        gripper_action = None
        if self.has_gripper:
       #      if via_point['gripper']:
            gripper_action = np.array([0.0084])
          #  else:
        #        gripper_action = np.array([-0.3])
        #     gripper_action = action[self.controller.control_dim:]  # all indexes past controller dimension indexes
            # gripper_action = np.array([-0.1])
        ...
        if policy_step:
            self.controller.set_goal(arm_action, self.desired_pos, self.desired_ori)
        else:
            self.controller.set_goal(arm_action, self.desired_pos, self.desired_ori)
            # self.controller.set_control_param(arm_action)
